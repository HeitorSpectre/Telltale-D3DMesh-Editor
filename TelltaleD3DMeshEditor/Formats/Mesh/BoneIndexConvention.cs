...
// across the body):
//   * v1 / v17 / v18 / v25 / v45 / v46: the parser already converts the raw byte to a DIRECT bone
//     index/palette index, so the value is used as-is. (v45 = MCSM Season 2, parsed by the Telltale
//     Toolkit which yields direct skeleton indices. Batman v46 stores the per-vertex bone as a plain palette byte,
//     exactly as the RTB importer reads it; dividing it by 3 drives each vertex from the wrong palette
//     slot, which mirrors left/right limbs and hides one side's bones in pose mode.)
...
public static class BoneIndexConvention
{
    public static int ToPaletteIndex(int rawBone, int meshVersion)
        => meshVersion is 1 or 17 or 18 or 25 or 45 or 46 ? rawBone
